...
async function main() {
  ...
  if (DEPLOYING) {
    ...
    const symbol = 'cUSDC-ARB-GMX'
    const newAsset = CONSTANTS.CHAIN_ID[42161].TOKEN_ADDRESSES.USDC
    const newIsNativeAsset = false
    const newPerformanceFee = ethers.parseUnits('0.2', 18)
    const newMaxCapacity = ethers.parseUnits('50', 6)
  ...
  console.log('Approve owner USDC allowance for lending vault');
  ...
  console.log('Deposit 3 USDC to Lending Vault');
